...
public:
  ...
  /**
   * @brief Converts a numerical type to a string (byte array, up to 8 bytes
   * long)
   * @param T numerical value
   * @return std::string
   */
  template <typename T> static std::string toByte(const T v) {
...
private:
  ...
  void readChunk(std::string& readData);
